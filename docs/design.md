...
└─ src/
   ...
   │  ├─ messages.ts            # バージョン付き P2P メッセージ型
   │  ├─ rtcConnection.ts       # WebRTC DataChannel の薄いラッパ
   │  ├─ sdp.ts                 # SDP の encode/decode（初版=Base64、将来=Munging）
   │  ├─ guestTransport.ts      # 子機側 Transport 実装
   │  └─ hub.ts                 # ホスト側の複数接続束ね（Star トポロジ管理）
   │
   ├─ app/                      # アプリ状態の結合層
   │  ├─ events.ts              # SessionEvent / FusionEvent
   │  ├─ session.ts             # GameSession 共通インターフェース
   │  ├─ localSession.ts        # ローカル = 子機ゼロのホスト
   │  ├─ hostSession.ts         # HostController + Hub の結線
   │  ├─ guestSession.ts        # GuestController + GuestTransport の結線
   │  ├─ hostController.ts      # ホスト：アクション受信→engine→状態配信
   │  ├─ guestController.ts     # 子機：入力送信／状態受信
   │  ├─ signaling.ts           # offer/answer 接続手順の状態機械
   │  └─ sfxBinding.ts          # SessionEvent → 効果音
   │
   ├─ ui/                       # プレゼンテーション
   │  ├─ partAssets.ts          # kind → 表示ラベル/よみ/画像 URL
   │  ├─ useNotice.ts           # notice 寿命管理
   │  ├─ screens/
   ...
   └─ test/
      ...
      ├─ controller.test.ts     # HostController
      ├─ guestController.test.ts# GuestController
      ├─ session.test.ts        # LocalSession
      ├─ signaling.test.ts      # 接続手順
      └─ sfxBinding.test.ts     # 効果音イベント
```
...
```ts
/** 漢字パーツ。id で同一性を判定し、kind でレシピ照合する */
...
export interface GameState {
  ...
  field: Part[];           // 現在の場札（通常 3 枚、パスで積み増し）
...
export function refillField(s: GameState): GameState;
...
// 手番を次へ進め、場札を山札から1枚積み増す（パス・合体失敗の両方で呼ぶ）
export function passTurn(s: GameState): GameState;

// 終了判定: 「手札 0 のプレイヤーが出た」/「山札空かつ場札空」/「山札空で誰も場札と合体できない詰み」
export function checkGameEnd(s: GameState): GameState; // phase/winnerId を確定
```

**場札は複数枚（`field: Part[]`、通常 `FIELD_SIZE = 3` 枚に補充）**。実機テストで「毎ターン 1 枚だけの場札が手札と合わずパスばかりになる」問題が出たため、場に常時 3 枚を並べてマッチ率を上げた。初期手札は 6 枚、レシピは 72 種。

さらに「3 枚並べてもパスが連鎖し、手札が場札と噛み合わず手詰まる」問題に対し、**点を取れずに手番が移るたび（自発的なパス・合体に失敗した提出のどちらでも）、その本人の手札と場札を山札から 1 枚ずつ積み増す**仕様にした（`passTurn`）。漢字は「場札＋手札」で成立するため、合体の両側に新しい候補が積み上がり手詰まりを直接解消する。場札は `FIELD_SIZE` を超えて増えうる。UI 側は同種の場札を重ねて枚数バッジで表示し、増えても散らからないようにする。山札が空のときは引かず手番だけ進む。
...
4. **失敗（合体不成立）**: 呼び出し側が `passTurn`（手番交代＋本人の手札と場札を山札から 1 枚ずつ積み増し）。

> 補給ルール: 正解時は手札・場札とも補給なしで同じプレイヤーが続けて行動（場札は使った 1 枚だけ補充）。パス／合体失敗時は、その本人の手札と場札を山札から 1 枚ずつ引いて手番交代。
> **終了条件（確定）**: 次のいずれかで `finished`。得点最多が勝者、タイブレークは手札の少なさ。
> 1. いずれかのプレイヤーの手札が 0 枚（仕様 2 の本来条件）。
> 2. 山札が空かつ場札も空（`deck.length === 0 && field.length === 0`）。
> 3. 山札が空で、どのプレイヤーの手札も場札と合体できない詰み状態（補充できず盤面が動かないため終了）。
...
仕様 4.2 を型化し、双方向に拡張する。全メッセージは `v: PROTOCOL_VERSION` を持ち、受信側は異なるバージョンを `deserialize` で拒否して安全に無視する。

```ts
export type NetMessage = { v: typeof PROTOCOL_VERSION } & (
  // Host → Guest（状態配信）
  | { type: 'WELCOME';       payload: { playerId: number; state: PublicGameState } }
  | { type: 'STATE_SYNC';    payload: PublicGameState }
  | { type: 'SUBMIT_RESULT'; payload: { playerId: number; field: string; part: string; result: string | null } }
...
> `STATE_SYNC` は確定状態のスナップショット、`SUBMIT_RESULT` は合体演出、`ACTION_RESULT` は通知・効果音用の行動結果。差分同期ではなく確定スナップショット配信にすることで、パケット欠落時の自己修復が容易になる。
...
QR: 生成は `qrcode`（[QrDisplay](../src/ui/components/QrDisplay.tsx)）、カメラ読取は `jsQR`（[QrScanner](../src/ui/components/QrScanner.tsx)）。いずれもオフライン動作で外部送信なし。ConnectScreen はコピペと QR を併用でき、どちらでも接続できる。offer/answer の生成・受理手順は `app/signaling.ts` に抽出され、UI は文字列表示・入力・QR 操作だけを担当する。
...
### 7.1 GameSession（モード Strategy）

UI は `GameSession` だけを操作する。ローカル/ホスト/ゲストの差は `app/` 層の実装に閉じ込める。
...
- `LocalSession`: `HostController` + Null transport。ローカル独自のルール実装を持たない。
- `HostSession`: `HostController` + `Hub`。
- `GuestSession`: `GuestController` + `GuestTransport`。

`SessionEvent` は `state` / `fusion` / `action-result` を UI へ届ける。通知は `ui/useNotice.ts`、効果音は `app/sfxBinding.ts` が購読する。

### 7.2 hostController.ts（権威ロジック）
...
[guest action 受信]
 ...
 checkGameEnd
      │
      ▼
 hub.broadcast(STATE_SYNC + ACTION_RESULT)  → 全子機 & 自分の UI 更新
```

ホストもプレイヤーの 1 人。自分の入力は内部で直接 engine に流す（ループバック）。

### 7.3 guestController.ts
