...
さらに「3 枚並べてもパスが連鎖する」問題に対し、**点を取れずに手番が移るたび（自発的なパス・合体に失敗した提出のどちらでも）、場札を引き直さず山札から 1 枚ずつ積み増す**仕様にした（`passTurn`）。手番が空回りするたびに合体候補が積み上がるため、いずれ誰かが出せる状態に近づく。場札は `FIELD_SIZE` を超えて増えうる。UI 側は同種の場札を重ねて枚数バッジで表示し、増えても散らからないようにする。
...
4. **失敗（合体不成立）**: 呼び出し側が `passTurn`（手番交代＋場札を山札から 1 枚積み増し）。

> 補給ルールは「手札への補給なし」で確定。場札は通常 3 枚に補充し、パス時は山札から 1 枚積み増す。正解時は同じプレイヤーが続けて行動する。
> **終了条件（確定）**: 次のいずれかで `finished`。得点最多が勝者、タイブレークは手札の少なさ。
> 1. いずれかのプレイヤーの手札が 0 枚（仕様 2 の本来条件）。
> 2. 山札が空かつ場札も空（`deck.length === 0 && field.length === 0`）。
> 3. 山札が空で、どのプレイヤーの手札も場札と合体できない詰み状態（パスで場札を増やせず盤面が動かないため終了）。
