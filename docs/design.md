...
さらに「3 枚並べてもパスが連鎖する」問題に対し、**パスでは場札を引き直さず、山札から 1 枚ずつ積み増す**仕様にした（`passTurn`）。パスのたびに合体候補が積み上がるため、いずれ誰かが出せる状態に近づく。場札は `FIELD_SIZE` を超えて増えうる。UI 側は同種の場札を重ねて枚数バッジで表示し、増えても散らからないようにする。
