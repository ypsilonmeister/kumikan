...
1. `playerId` が手番か検証（不正なら `fail`）。
2. 場札 `field` と提示パーツの `kind` で `checkCombination`。
3. **成功**: 漢字を `players[pid].score` に追加、手札から該当 Part 除去、場札クリア。補給は行わず、同じプレイヤーが続けて行動する。
4. **失敗**: 状態変更なし、`outcome: 'fail'`。
5. 呼び出し側（hostController）が `checkGameEnd` を適用し、成功時は手番据え置きで `drawField`、失敗・パス時のみ `nextTurn` を適用。

> 補給ルールは「補給なし」で確定。正解時は同じプレイヤーが続けて行動する。
> **終了条件（確定）**: 次のいずれかで `finished` とし、得点（完成漢字の数）最多を勝者とする。タイブレークは手札の少なさ。
> 1. いずれかのプレイヤーの手札が 0 枚（仕様 2 の本来条件）。
> 2. 山札が空かつ場札も出せない（`deck.length === 0 && field === null`）。手番開始時に新しい場札をめくれず手番が回らなくなるため、終了とする。
