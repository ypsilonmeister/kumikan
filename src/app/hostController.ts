...
export interface HostCallbacks {
  ...
  /** 成功/失敗/パスの結果。通知・効果音の単一チャネルとして使う。 */
...
export class HostController {
  ...
  private welcome(peerId: number): void {
    this.transport.sendTo(peerId, netMessage({
      type: 'WELCOME',
  ...
  private broadcastState(): void {
    for (const player of this.state.players) {
      ...
      this.transport.sendTo(player.id, netMessage({
        type: 'STATE_SYNC',
    ...
    if (this.state.phase === 'finished') {
      this.transport.send(netMessage({ type: 'GAME_OVER', payload: { winnerId: this.state.winnerId } }));
    }
  }

  /** 指定プレイヤーとしてのパーツ提示（ホスト自身は playerId=0、ローカルは手番プレイヤー）。 */
  submit(playerId: number, partId: string, fieldPartId?: string): void {
    this.applySubmit(playerId, partId, fieldPartId);
  }
  ...
  private applySubmit(playerId: number, partId: string, fieldPartId?: string): void {
    ...
    if (result.outcome === 'success' && result.kanji) {
      ...
      this.transport.send(netMessage({
        type: 'SUBMIT_RESULT',
        payload: {
          playerId,
      ...
      this.broadcastState();
      this.publishActionResult({ kind: 'fused', playerId, playerName, char: result.kanji.char });
      return;
    }
    ...
    this.broadcastState();
    this.publishActionResult({
      kind: 'submit-failed',
      playerId,
      playerName,
      drew: this.state.deck.length < before.deck.length,
    });
  }
  ...
  private applyPass(playerId: number): void {
    ...
    const before = this.state;
    const playerName = this.state.players.find((item) => item.id === playerId)?.name ?? '';
    this.state = passTurn(this.state);
    this.broadcastState();
    this.publishActionResult({
      kind: 'passed',
  ...
  private publishActionResult(result: ActionResult): void {
    this.callbacks.onActionResult?.(result);
    this.transport.send(netMessage({ type: 'ACTION_RESULT', payload: result }));
  }
